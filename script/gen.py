...
p.add_argument('-db', default="gsb", type=str, help="db")
p.add_argument('-t', default="uc_User", type=str, required=False, help='-t=role,ucUser')
...
def get_java_field(table_name):
    cloumns = get_column(table_name)
    new_cloumns = []
    for row in cloumns:
...
def gen_code(table_name):
    ...
    root_path = "/Users/luqingrun/IdeaProjects/gongsibao/"
    ...
    source_file = root_path + "gongsibao-service/src/main/java/com/gongsibao/module/"
    ...
    controller_path = root_path + get_file_path(get_module_name(table_name)) + "/src/main/java/com/gongsibao/"+ get_module_name(table_name) +"/controllers/" + get_java_name(
        table_name).lower() + "/"
